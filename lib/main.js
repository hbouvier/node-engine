(function () {
    ...
    if (!options.script ||options.script === '') {
        logger.log('error', '%s|USAGE|[--workers #] [--port #] [--level={debug|info|warn|error}] --script=path/script.js', config.name, meta);
        process.exit(-1);
    }
    ...
    if (unknownArgs) {
        logger.log('error', '%s|Unknown arguments|"' + unknownArgs.join('", "') + '"', config.name, meta);
        logger.log('error', '%s|USAGE|[--workers=#] [--port=#] [--level={debug|info|warn|error}] --script=path/script.js', config.name, meta);
        process.exit(-1);
    }
