...
function Queue(name, options) {
    ...
    this.executorFunc = options && typeof(options.execute) === 'function' ?
        ...
        function (err, node, next) {
            logger.log('debug', 'Queue::defaultExecutor|node=' + util.inspect(node) + '|next=' + next);
...
Queue.prototype.add = function (node) {
    logger.log('debug', 'Queue|'+this.name+'|add|node=' + util.inspect(node), meta);
...
Queue.prototype.execute = function (key, next) {
    ...
    if (!key || (typeof(key) === 'function' && !next)) {
        ...
        if (node) {
            logger.log('debug', 'Queue|'+this.name+'|execute|executor|node=' + util.inspect(node), meta);
    ...
    if (node) {
        logger.log('debug', 'Queue|'+this.name+'|execute|re-inserting|node=' + util.inspect(node), meta);
