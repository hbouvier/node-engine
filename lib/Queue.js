...
function Queue(name, options) {
    ...
    this.executorFunc = options && typeof(options.execute) === 'function' ?
        ...
        function (err, node, next) {
            logger.log('debug', 'Queue::defaultExecutor|node=' + util.inspect(node, {depth:0,colors:true}) + '|next=' + next);
...
Queue.prototype.add = function (node) {
    logger.log('debug', 'Queue|'+this.name+'|add|node=' + util.inspect(node, {depth:1,colors:true}), meta);
...
Queue.prototype.execute = function (key, next) {
    ...
    logger.log('debug', 'Queue|'+this.name+'|execute|' + (!key || typeof(key) === 'function' ? 'next' : ('key='+key+'|next')) + '|len=' + this.queue.length, meta);
    if (!key || (typeof(key) === 'function' && !next)) {
        next = !key ? next : key;
        node = this.isEmpty() ? null : this.queue.shift();
        if (node) {
            logger.log('debug', 'Queue|'+this.name+'|execute|executor|node=' + util.inspect(node, {depth:0,colors:true}), meta);
    ...
    if (node) {
        logger.log('debug', 'Queue|'+this.name+'|execute|re-inserting|node=' + util.inspect(node, {depth:0,colors:true}), meta);
        this.queue.unshift(node);
    }
    if (this.isEmpty() === false)
        this.emit('execute', node);
    logger.log('debug', 'Queue|'+this.name+'|execute|len=' + this.queue.length, meta);
};
...
Queue.prototype.isEmpty = function() {
    logger.log('debug', 'Queue|'+this.name+'|isEmpty|len=' + this.queue.length + '|' + (this.queue.length === 0), meta);
...
/**
 * Add a listener on the possible events for the queue : 'add', 'execute'
 *
 * @param event
 * @param callback
 * @returns {*}
