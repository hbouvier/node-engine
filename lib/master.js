...
module.exports = function (config) {
    ...
    function restartAction(params) {
        ...
        var cycleWorkers = function (workersToShutdown) {
            ...
            startPromise.then(function (object) {
                ...
                actionCompleted("restart", states.error, false, 'A worker failed to start', params);                        
            }).done();
        };
    ...
    function setupObservers() {
        ...
        cluster.on('exit', function(worker, code, signal) {
            ...
            // TODO: When restarting we should check if it was an old worker diying or a new one
            //       if it is a new one, we should NOT call workerHelperDone!!!
            if (state === states.stopping || state === states.restarting) {
                workerHelperDone('exited', worker);
            } else if (state === states.running) {
                logger.log('error', 'M|worker-id=%d|died|resting', newWorker.id, meta);
                var newWorker    = startOneWorkerHelper();
                var startPromise = monitorOneWorkerHelper("start", savedConfig.startTimeoutInMs, newWorker);
                ...
                startPromise.then(function (object) {
                    ...
                    logger.log('error', 'M|worker-id=%d|restart|FAILED', newWorker.id, meta);
                });
            }
        });
