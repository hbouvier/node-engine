module.exports = function (config) {
    ...
    var startTimeoutInMs = 30000,
        actionQueue = new Queue('actionQueue', function (key, node) {
                return true;
            },
        ...
        startQueue = new Queue('startQueue', function (key, node) {
                return (node.id === key);
            },
            function (err, node, next) {
                return err ? err : next(err, node);
            }
        ),
        stopQueue = new Queue('stopQueue', function (key, node) {
                return (node.id === key);
            },
            function (err, node, next) {
                return err ? err : next(err, node);
    ...
    function startAction(params) {
        ...
        if (params.options && params.options.watch) {
            if (typeof(params.options.watch) === 'string') {
